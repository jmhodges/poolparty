...
describe "Ec2RemoteInstance" do
  ...
  it "should have the name set" do
    ...
    # @ec2.respond_to?(:name).should == true #MF name method should be defined, possibly thru delegator
  end
  it "should raise an error if not passed a node identifier" do
    lambda {TestEc2RemoteInstance.new( {:useless => "frank"} )}.should raise_error
    lambda {TestEc2RemoteInstance.new }.should raise_error
  end
